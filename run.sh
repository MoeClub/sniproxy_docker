...
[ -f "/etc/ocserv/ocpasswd" ] || {
  for gp in `find /etc/ocserv/group -type f 2>/dev/null`; do
    ...
    user=""
    passwd=""
    [ -n "$gv" ] && user=`echo "$gv" |cut -d: -f1`
    [ -n "$gv" ] && passwd=`echo "$gv" |cut -d: -f2`
    [ -n "$user" ] || user=`openssl rand -base64 32 | tr -dc 'a-z' | head -c 3`
    [ -n "$passwd" ] || passwd=`openssl rand -base64 32 | tr -dc 'a-z' | head -c 3`
